...
$sql = "SELECT  Email FROM tblklant";	
$users =$dbh->query($sql);

$Email = $_POST['Email'];


if(isset($_POST['wijzigknop1'])) 
        {
  //connectie
    $dbh = new PDO (DB_CONNECTION, DB_USERNAME, DB_PASSWORD);
    // als e-mail bestaat niet updaten in databank en foutmelding tonen
    
foreach($users as $user)
{
if(strtoupper($_POST['Email']) != strtoupper( $_SESSION['Email']) &&
   strtoupper($_POST['Email']) == strtoupper($user['Email']))
   
{
        $_SESSION['ALERT_CODE'] = 'ERROR';
        $_SESSION['ALERT_HEAD'] = 'Updaten mislukt!';
  ...
  if ($_POST['Wachtwoord'] != $_SESSION['bolletjes']) { 
            ...
            $statement->bindvalue(":Wachtwoord",password_hash( strtoupper($_POST['Email']).$_POST['Wachtwoord'],PASSWORD_DEFAULT ));
            ...
            $statement->bindvalue(":Email",$_POST['Email']);
            $statement->bindvalue(":KlantID",$_SESSION['KlantID']);
            $statement->bindvalue(":Wachtwoord",password_hash( strtoupper($_POST['Email']).$_SESSION['Wachtwoord'],PASSWORD_DEFAULT ));
            $statement->execute();
          }    
